...
public:
     ...
     * @param db           Reference to your blocksDB (for metadata lookups and persistence).
     ...
     */
    Block produce(blocksDB &db, int txnCount, const std::string &testFilePath,GlobalState& gs) {
        ...
        while (transactions.size() < static_cast<size_t>(txnCount) && std::getline(infile, line)) {
            ...
            } else if (exe == "ecommClientMain" ||exe == "eCommClientMain" || exe == "./ecommClientMain"|| exe == "./eCommClientMain") {
                eCommClient eComm;
        ...
        if (transactions.empty()) {
            std::cerr << "No transactions parsed; returning empty block." << std::endl;
            return {};
        }
