...
int main() {
  ...
  for (int count = 0; count < blocksCount; ++count) {
    ...
    scheduler parallelScheduler(state);
    auto start = std::chrono::high_resolution_clock::now();
    if(count%2 == 0){
    cout << "Setup File Running :" << endl ;
    newBlock = text.produce(db,txnCount,"../testFile/setup.txt",state) ;
         }
         else{
          cout << "Test File Running :" << endl ;
          newBlock = text.produce(db,txnCount,"../testFile/testFile.txt",state) ;
