...
TEST(DAGmoduleTest, CompleteTransaction) {
  ...
  // Complete transaction -1 (non-existent)
  dag.complete(-1);
  
  // Complete transaction 0 again
  dag.complete(0);

  // Validate the in-degree updates after completing -1 and 0 again
  EXPECT_EQ(dag.inDegree[2].load(), 1);
}
